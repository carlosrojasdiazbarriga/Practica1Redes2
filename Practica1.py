...
class PracticaUnoGUI:
    ...
    def guardar_reporte(self):
        ...
        if encontrado:
            ...
            so = self.consultaSNMP(comunidad,host,"1.3.6.1.2.1.1.5.0")
            ...
            num_interfaces = int(self.consultaSNMP(comunidad, host, "1.3.6.1.2.1.2.1.0"))
            for i in range(1, num_interfaces + 1):
                # Obtener la descripción y estado de la interfaz
                descr_bytes = self.consultaSNMP(comunidad, host,"1.3.6.1.2.1.2.2.1.2."+str(i))[3:-2]
                ...
                estado = self.consultaSNMP(comunidad, host, f"1.3.6.1.2.1.2.2.1.8.{i}")
                estados = {
                1: "up",
                2: "down",
                3: "testing",
            ...
            #     cuentaIn = self.consultaSNMP(comunidad,host,'1.3.6.1.2.1.2.2.1.7.' + str(cont))
            ...
            #     if so.find("Linux") != -1:
            #         descripcion = self.consultaSNMP(comunidad,host, "1.3.6.1.2.1.2.2.1.2"+str(a))
            #     else:
            #         otros = self.consultaSNMP(comunidad,host,"1.3.6.1.2.1.2.2.1.2"+str(a))[3:]
            #         descripcion = bytes.fromhex(otros).decode('UTF-8')
            #     statusInterface=self.consultaSNMP(comunidad,host,"1.3.6.1.2.1.2.2.1.7"+str(a))
            ...
            elementos.append(Paragraph("Carlos Rojas Diaz Barriga Practica 1"))

            elementos.append(Spacer(1, 12))
            # Agregar la información general del dispositivo
            elementos.append(Paragraph(f"Sistema operativo: {so}"))
